...
describe('AttendancePage - Comprehensive Tests', () => {
  ...
  it('should handle different time formats', async () => {
    const timeFormatSubscriptions = [
      ...
      },
      {
        ...
        nom: 'Bernard',
        prenom: 'Pierre',
        tarif: 'SAMEDI 10h Paris Châtelet ADULTES'
      }
    ]

    ;(global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve(timeFormatSubscriptions)
    })
  ...
  it('should handle Choisy le Roi courses', async () => {
    const choisySubscriptions = [
      {
        _id: '1',
        nom: 'Dupont',
        prenom: 'Jean',
        tarif: 'SAMEDI 16h30 Choisy le Roi ADULTES'
      },
      {
        _id: '2',
        nom: 'Martin',
        prenom: 'Marie',
        tarif: 'DIMANCHE 11h30 Choisy le Roi ENFANTS'
      }
    ]

    ;(global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve(choisySubscriptions)
    })
